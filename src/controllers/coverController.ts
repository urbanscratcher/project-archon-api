...
export const createCover = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
    ...
    if (!coverReq.success) throw new BadRequestError("req body is not valid");
    ...
    // when duplicated
    const existingCovers = await conn.query(
      `SELECT * FROM COVER WHERE insight_idx = ?`,
      insightIdx
    );
...
export const updateCover = asyncHandledDB(
  async (conn: any, req: Request, res: Response, next: NextFunction) => {
    ...
    const mainCovers = await conn.query(
      `SELECT * FROM COVER WHERE is_main = 1`
    );
...
export const removeCover = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
    const idx = validateParamIdx(req);
...
export const getAllCovers = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
...
export const getHeadline = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
