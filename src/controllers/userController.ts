...
class UserDto extends Dto {
  ...
  constructor(obj: Array<any>) {
    ...
    this.careers = obj[0].careers
      ? obj[0].careers.replace(/"/g, "'")
      : undefined;
...
export const createUser = asyncHandledDB(
  async (conn: any, req: Request, res: Response, next: NextFunction) => {
    ...
    const emails = await conn.query(
      `SELECT * FROM USER WHERE email = ? AND del_at is null`,
      user.email
    );
    if (emails.length > 0) {
    ...
    , created_at = ?`,
        [
          ...
          user.jobTitle,
          user.biography,
          user.careers ? "'" + JSON.stringify(user.careers) + "'" : null,
          toMysqlDate(),
        ]
      );
      ...
      if (user.topics) {
        ...
        user.topics.forEach(async (topicIdx: number) => {
          const cateResult = await conn.query(
            `INSERT INTO USER_TOPIC (user_idx, topic_idx) values (?, ?)`,
            [insertedIdx, topicIdx]
          );
...
export const getUsers = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
    ...
    const userList: ListDto<any> = new ListDto(
      usersWithExtraInfo,
      total,
      query.offset,
      query.limit
...
export const getUser = asyncHandledDB(
  async (conn: any, req: Request, res: Response) => {
...
export const deleteUser = asyncHandledDB(
  async (conn: any, req: Request, res: Response, _next: NextFunction) => {
    // parse
    const idx = validateParamIdx(req);
    ...
    // DB
    const result = await conn.query(
      `UPDATE USER SET del_at = ? WHERE idx = ?`,
      [toMysqlDate(), idx]
    );
    logger.debug({ res: result }, "DB response");
...
export const updateUser = asyncHandledDB(
  async (conn: any, req: Request, res: Response, next: NextFunction) => {
    const idx = validateParamIdx(req);
    ...
    if (
      (newUser.pastPassword ||
        newUser.newPassword ||
        newUser.newPasswordConfirm) &&
      !(
        newUser.pastPassword &&
        newUser.newPassword &&
        newUser.newPasswordConfirm
      ...
      throw new BadRequestError(
        "all password info(past password, new password, new password confirm) required"
      );
    }
    ...
    if (
      ...
      const isMatched = await bcrypt.compare(
        decryptedPastPassword,
        oldUser.password
      );
      if (!isMatched) {
        throw new UnprocessableError(
          "passwords are not matched with the past one. cannot be updated"
        );
      }
      ...
      if (newUser.pastPassword === newUser.newPassword) {
        throw new DuplicationError(
          "new password should not be the same with the existing one"
        );
      }
    }
    ...
    const hashedPassword =
      decryptedNewPassword !== ""
        ? await bcrypt.hash(decryptedNewPassword, 12)
        : null;
    ...
    WHERE idx = ?`,
        [
          ...
          newUser.jobTitle === ""
            ? null
            : newUser.jobTitle === undefined
            ? oldUser.jobTitle
            : newUser.jobTitle,
          newUser.biography === ""
            ? null
            : newUser.biography === undefined
            ? oldUser.biography
            : newUser.biography,
          hashedPassword ?? oldUser.password,
          newUser.careers
            ? newUser.careers.length > 0
              ? JSON.stringify(newUser.careers)
              : null
            : oldUser.careers,
    ...
    if (newUser.topics) {
      ...
      for (const topicIdx of newUser.topics) {
        const topics = await conn.query(
          `SELECT idx FROM TOPIC WHERE idx = ?`,
          topicIdx
        );
        if (topics.length <= 0) {
      ...
      try {
        ...
        for (const topicIdx of newUser.topics) {
          await conn.query(
            `INSERT INTO USER_TOPIC SET user_idx = ?, topic_idx = ?`,
            [idx, topicIdx]
          );
        }
        await updateUser();
